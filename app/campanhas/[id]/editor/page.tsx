...
import { getFormatosPorPlataforma, getConfigFormato } from "@/lib/config-plataformas"
...
export default function EditorCampanhaPage() {
  ...
  const { elementos, setElementos, resetElementos, salvarVersao, versoes } = usePecasStore()
  const { campanhas, campanhaAtual, selecionarCampanha, adicionarAsset, obterAsset, atualizarCampanha } =
    useCampanhasStore()
  ...
  useEffect(() => {
    if (campanhaId && campanhas.length > 0 && !campanhas.some((c) => c.id === campanhaId)) {
      toast({
  ...
  useEffect(() => {
    ...
    if (campanhaAtual && !campanhaAtual.plataformas.includes(selectedPlatform)) {
  ...
  useEffect(() => {
    if (selectedFormat) {
      ...
      if (campanhaId) {
        const assetSalvo = obterAsset(campanhaId, selectedPlatform, selectedFormat)
  ...
  }, [selectedFormat, selectedPlatform, resetElementos, campanhaId, obterAsset, setElementos])

  // Auto-save quando os elementos mudam (com debounce)
  useEffect(() => {
    if (!autoSave || !campanhaId || !selectedPlatform || !selectedFormat || !formatConfig) return
  ...
  const handleAplicarElementos = (elementosCompartilhados: any[]) => {
    try {
      ...
      const elementosAdaptados = adaptarElementosParaFormato(elementosCompartilhados, formatConfig)
      console.log("Elementos adaptados para o formato atual:", elementosAdaptados)
  ...
  const adaptarElementosParaFormato = (elementos: any[], formatoConfig: any) => {
  ...
  return (
    <div className="container px-4 py-6 mx-auto md:px-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">{campanhaAtual ? `Editor: ${campanhaAtual.nome}` : "Editor de Peças"}</h1>
        <div className="flex items-center gap-2">
          <div className="text-sm text-gray-500">{lastSaved && <span>Último salvamento: {lastSaved}</span>}</div>
      ...
      {!campanhaAtual && (
        <Alert className="mb-6">
          ...
          <AlertDescription>
            A campanha que você está tentando editar não existe ou não foi carregada corretamente.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <Button className="w-full" variant="outline" onClick={handleSaveVersion} disabled={!formatConfig}>
              Salvar Versão Atual
            </Button>
          ...
          <div className="p-4 border rounded-lg">
            ...
            <div className="space-y-2">
              <p className="text-sm text-gray-500 mb-3">Salve e reutilize elementos entre diferentes formatos.</p>
        ...
        <div className="lg:col-span-9">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="elementos">
              {formatConfig ? (
                <EditorElementos formatConfig={formatConfig} elementos={elementos} setElementos={setElementos} />
              ) : (
                <div className="flex items-center justify-center h-64 text-gray-500">
