...
export function RenderizadorElemento({
  ...
  }, [posicaoGrid, gridConfig.columns, gridConfig.rows, isEditing, isDragging, camada, estilo])
  ...
  useEffect(() => {
    ...
    const xLines = Array.from({ length: gridConfig.columns + 1 }).map((_, i) => i * cellWidth)

    // Generate Y positions for each horizontal grid line
    const yLines = Array.from({ length: gridConfig.rows + 1 }).map((_, i) => i * cellHeight)
  ...
  const handleMouseUp = useCallback(() => {
    if (!isDragging || !container || !elementRef.current || !onPositionChange) return
    ...
    const gridX = Math.max(0, Math.min(Math.round(x / cellWidth), gridConfig.columns - posicaoGrid.width))
    const gridY = Math.max(0, Math.min(Math.round(y / cellHeight), gridConfig.rows - posicaoGrid.height))
    ...
    elementRef.current.style.width = `${(posicaoGrid.width / gridConfig.columns) * 100}%`
    elementRef.current.style.height = `${(posicaoGrid.height / gridConfig.rows) * 100}%`
  }, [isDragging, container, gridConfig, onPositionChange, id, posicaoGrid.width, posicaoGrid.height])
  ...
  const renderContent = () => {
    switch (tipo) {
      case "logo":
        return (
          <div
            className={cn("flex items-center justify-center relative w-full h-full", isDragging && "opacity-70")}
            onClick={onClick}
          >
            {conteudo.src ? (
              <Image
      ...
      case "texto":
        return (
          <div
            className={cn("overflow-hidden w-full h-full", isDragging && "opacity-70")}
            style={{
      ...
      case "imagem":
        return (
          <div className={cn("overflow-hidden relative w-full h-full", isDragging && "opacity-70")} onClick={onClick}>
            {conteudo.src ? (
              <Image
                src={conteudo.src || "/placeholder.svg"}
      ...
      case "imagemPersonalizada":
        return (
          <div
            className={cn("overflow-hidden relative w-full h-full", isDragging && "opacity-70")}
            onClick={onClick}
            style={{
              borderRadius: conteudo.borda ? conteudo.bordaRadius || "0px" : "0px",
      ...
      case "cta":
        return (
          <div
            className={cn("flex items-center justify-center w-full h-full", isDragging && "opacity-70")}
            onClick={onClick}
          >
            <div
      ...
      default:
        return (
          <div
            className={cn(
              "flex items-center justify-center bg-gray-100 text-gray-500 text-sm w-full h-full",
